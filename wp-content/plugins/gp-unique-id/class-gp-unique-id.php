...
class GP_Unique_ID extends GP_Plugin {
	...
	public function get_unique( $form_id, $field, $length = 5, $atts = array(), $entry = false, $check_spam = true ) {
		...
		if ( $check_spam ) {
			...
			$gpui_spam_cache_key = "gpui_is_spam_{$form_id}_{$entry_id}";

			$found = null;

			// Check if we found the cache for an entry.
			$is_spam = GFCache::get( $gpui_spam_cache_key, $found, false );
			...
			if ( ! $found ) {
				 ...
				 * payment).
				 *
				 * A second honeypot validation was happening because this check (GFCommon::is_spam_entry()) was running again
				 * during the `gform_post_add_entry` hook. This gets triggered when payment gateways like Stripe update the entry
				 * via GFAPI, which unintentionally causes the honeypot hash check to fail at a later point in the process.
				 *
				 * If we run into future issues with this, it'd be worth exploring whether we can check the `status` of
				 * the entry rather than calling the GFCommon::is_spam_entry() method which has been problematic in the past.
				 */
				$is_spam = $entry && ! rgar( $form, 'enableHoneypot' ) && ! rgar( $entry, 'transaction_id' ) && GFCommon::is_spam_entry( $entry, $form );
				...
				GFCache::set( $gpui_spam_cache_key, $is_spam );
			}
