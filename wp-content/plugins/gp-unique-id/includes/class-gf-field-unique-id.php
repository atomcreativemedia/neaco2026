...
class GF_Field_Unique_ID extends GF_Field {
	...
	public function populate_field_value( $entry, $form, $fulfilled = false ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			GFAPI::update_entry_field( $entry['id'], $field['id'], self::$_generated_uids[ $entry['id'] ][ $field->id ] );
		}
	...
	public function delayed_payment_populate_field_value( $transaction_id, $payment_feed, $entry, $form ) {
		// Only populate if it's not already populated.
		$all_uid_fields_populated = true;
		foreach ( $form['fields'] as $field ) {
			if ( $field->type == 'uid' && empty( $entry[ $field->id ] ) ) {
				$all_uid_fields_populated = false;
				break;
			}
		}

		if ( $all_uid_fields_populated ) {
			return;
		}
	...
	public function save_value( $entry, $field, $value = null ) {
		...
		// Ensure if the Unique ID is generated/stored (based on 'result'), save it to the static store.
		self::$_is_gpuid_generated = $result;
		if ( $result ) {
			self::$_generated_uids[ $entry['id'] ][ $field->id ] = $value;
		}
